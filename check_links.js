...
(async () => {
    ...
    const modelPattern = /<a\s+href="(D2301\.do\?[^"]+)"[^>]*>([\s\S]*?)<\/a>/;
    const match = modelPattern.exec(portalHtml);
    if (!match) return console.log('No machine found');
    
    const url = match[1].replace(/&amp;/g, '&');
    console.log(`Machine URL: ${url}`);
    
    console.log('Fetching Machine Page...');
    const machineHtml = await fetchHTML(url);
    const fs = require('fs');
    fs.writeFileSync('machine_debug.html', machineHtml, 'utf8');
    console.log('Saved to machine_debug.html');
    
    const linkPattern = /<a\s+href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/g;
    let lMatch;
    while((lMatch = linkPattern.exec(machineHtml)) !== null) {
